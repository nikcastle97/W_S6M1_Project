...
export default function Programmers() {
  ...
  const [proList, setProList] = useState(listOfAwesome);
  const [selPro, setPro] = useState('');
  // console.log(proList)
  ...
  const getNameOfFeatured = () => {
    ...
    return proList.find(pro => pro.id == selPro).name
  };
      ...
      <div className='programmers'>
        {
          ...
          proList.map(dev =>
            <div className='programmer' key={dev.id}>
              {dev.name} <button onClick={() => {setPro(dev.id)}}>Feature</button>
            </div>
          )
        }
      </div>
